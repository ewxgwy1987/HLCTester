...
{
    public class PLC2GWSessionForward : PALS.Net.Handlers.SessionHandler
    {
        ...
        /// Class constructer.
        /// </summary>
        public PLC2GWSessionForward(PALS.Common.IParameters Param)
        {
            if (!Init(ref Param))
                throw new Exception("Creating class " + _className +
                    " object failed! <BHS.PLCSimulator.PLC2GWSessionForward.Constructor()>");
        }

        /// <summary>
        /// Class destructer.
        /// </summary>
        ~PLC2GWSessionForward()
        {
            Dispose(false);
        }
            ...
            try
            {
                ...
                if (count == 0)
                {
                    if (_logger.IsErrorEnabled)
                        _logger.Error("No connection is opened to Gateway! Message will be discarded. [Msg(APP):" +
